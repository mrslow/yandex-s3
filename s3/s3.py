...
from .core import YandexS3Client

__all__ = 'S3Client', 'S3Config', 'S3File'

# rounding of download link expiry time,
# this allows the CDN to efficiently cache download links
expiry_rounding = 100

# removing xmlns="http://s3.amazonaws.com/doc/2006-03-01/"
# from xml makes it much easier to parse
...
class S3Client:
    ...
    def __init__(
        ...
        self._client = YandexS3Client(self._config)
    ...
    async def list(self, prefix: Optional[str] = None) -> AsyncIterable[S3File]:
        ...
        while True:
            ...
            if ((t := xml_root.find('IsTruncated')) is not None and
                    t.text == 'false'):
                break
    ...
    async def download(self, file_path: str) -> bytes:
        ...
        url = self.signed_download_url(file_path)
        r = await self._client.client.get(url)
    ...
    async def _delete_1000_files(self, *files: Union[str, S3File]) -> List[str]:
        ...
        assert len(files) <= 1000, \
               f'function can delete 1000 files max, not {len(files)}'
        objects = [f"<Object><Key>{to_key(k)}</Key></Object>" for k in files]
        xml = (
    ...
    def signed_upload_url(
        ...
        *,
        ...
        assert path == '' or path.endswith('/'), \
                'path must be empty or end with "/"'
        ...
        fields.update(Policy=b64_policy.decode(),
                      Signature=self._signature(b64_policy))
        return dict(url=f'https://{self._client.host}/', fields=fields)

    def _signature(self, to_sign: bytes) -> str:
        s = hmac.new(self._config.secret_key.encode(),
                     to_sign,
                     hashlib.sha1).digest()
        return base64.b64encode(s).decode()

    async def close(self):
        if not self._client.client.is_closed:
